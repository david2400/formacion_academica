 ...
 *
 * <p>Los estados admitidos no están fijados aquí: se parametrizan en
 * {@code /estados/contextos/estudiante_grupo}. El cliente consulta ese catálogo para
 * pintar filtros y selectores, y devuelve el {@code estado_id} elegido.
 */
...
public class EstudianteGrupoController {
    ...
    @Operation(summary = "Asignar estudiante a grupo", description = "Crea la asignación con el estado inicial parametrizado para el contexto. "
            + "Si el estudiante ya estuvo en el grupo y fue removido, reactiva la asignación existente en lugar de duplicarla.")
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Asignación creada", content = @Content(mediaType = "application/json", schema = @Schema(implementation = EstudianteGrupo.class))),
            @ApiResponse(responseCode = "400", description = "Datos inválidos", content = @Content),
            @ApiResponse(responseCode = "409", description = "El contexto no tiene estado inicial parametrizado", content = @Content),
            @ApiResponse(responseCode = "500", description = "Error interno", content = @Content)
    })
    @PostMapping
    public ResponseEntity<EstudianteGrupo> asignar(@Valid @RequestBody AsignarEstudianteGrupoDto request) {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Asignación encontrada", content = @Content(mediaType = "application/json", schema = @Schema(implementation = EstudianteGrupo.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Listado de asignaciones", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = EstudianteGrupo.class)))),
            @ApiResponse(responseCode = "500", description = "Error interno", content = @Content)
    })
    @GetMapping
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Asignaciones del grupo", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = EstudianteGrupo.class)))),
            @ApiResponse(responseCode = "500", description = "Error interno", content = @Content)
    })
    ...
    @Operation(summary = "Cambiar estado", description = "Actualiza el estado de la asignación. El estado debe estar habilitado "
            + "para el contexto 'estudiante_grupo'; de lo contrario se rechaza.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Estado actualizado", content = @Content(mediaType = "application/json", schema = @Schema(implementation = EstudianteGrupo.class))),
            @ApiResponse(responseCode = "400", description = "Estado no habilitado para el contexto", content = @Content),
            @ApiResponse(responseCode = "404", description = "Asignación no encontrada", content = @Content),
            @ApiResponse(responseCode = "500", description = "Error interno", content = @Content)
    })
    @PutMapping("/estado")
    public ResponseEntity<EstudianteGrupo> cambiarEstado(@Valid @RequestBody CambiarEstadoEstudianteGrupoDto request) {
        return ResponseEntity.ok(cambiarEstadoUseCase.cambiarEstado(request));
    }

    @Operation(summary = "Eliminar asignación", description = "Remueve al estudiante del grupo. Es un borrado lógico: "
            + "si se le vuelve a asignar al mismo grupo, la asignación se reactiva.")
    @ApiResponses({
